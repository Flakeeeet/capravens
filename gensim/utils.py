...
def extract_code(res):
    ...
    if res[1]=="`" and res[l-2]=="`":
        pattern = r'```(.*?)```'
    elif res[1]=="`" and res[l-2]=='"':
        pattern = r'```(.*?)"""'
    elif res[1]=='"' and res[l-2]=="`":
        pattern = r'"""(.*?)```'
    elif res[1]=='"' and res[l-2]=='"':
        pattern = r'"""(.*?)"""'    
    elif res[1]=="`" and res[3]=='p':
        pattern = r'```python(.*?)$```'
    else:
        pattern = r'```(.*?)$```'
    '''
...
def extract_list(res, prefix="code_reference"):
    ...
    if len(code_string) == 0:
        pattern = r'\d+\.\s*(.*?)\n'
        code_string = re.findall(pattern, res)
        if len(code_string) != 0:
            return prefix + '=' + str(code_string)
        else:
            pattern = r'\-\s*(.*?)\n'
            code_string = re.findall(pattern, res)
            return prefix + '=' + str(code_string)
...
def save_stat(cfg, output_dir, env_names, syntax_rate, run_rate, env_success_rate,
              diversity_score):
    """ save run results """
    print("=========================================================")
    print(
        f"{cfg['prompt_folder']} | TOTAL SYNTAX_PASS_RATE: {syntax_rate * 100:.1f}% RUNTIME_PASS_RATE: {run_rate * 100:.1f}% ENV_PASS_RATE: {env_success_rate * 100:.1f}% DIVERSITE SCORE: {diversity_score:.3f}")
    print("=========================================================")
    ...
    with open(os.path.join(output_dir, "eval_results.csv"), "w") as f:
        ...
        for col, stat in zip(["syntax", "runtime", "env. completion"],
                             [syntax_rate, run_rate, env_success_rate]):
...
def truncate_message_for_token_limit(message_history, max_tokens=13000):
    truncated_messages = []
    tokens = 0
...
def generate_feedback(prompt, max_tokens=2048, temperature=0.0, interaction_txt=None, retry_max=5,
                      n=1):
    """ use GPT-4 API """
    time.sleep(30)
    ...
    if model == "text-davinci-003":
        ...
        return generate_feed_back_llm(prompt, max_tokens, temperature)
        # return generate_feedback_api_llm(prompt, max_tokens, temperature)
    # print('PROMPT')
    # print(prompt)
    # print('---------------------------------------------------')
...
def format_finetune_prompt_codeonly(task_name,
                                    prompt_file='finetune_instructions_prompt_codeonly.txt'):
...
def generate_feedback_completion_only(prompt, max_tokens=800, temperature=0.0,
                                      interaction_txt=None, retry_max=5, n=1):
    """ use GPT-4 API """
...
def generate_feedback_completion_llm(prompt, max_tokens, temperature, interaction_txt=None,
                                     retry_max=5):
    '''use mistal or other hf models'''
    ...
    for retry in range(retry_max):
        try:
            ...
            llm_model_inputs = tokenizer(prompt, return_tensors="pt").to(device)
            llm_model.to(device)
...
def generate_feed_back_llm(prompt, max_tokens, temperature, interaction_txt=None, retry_max=5):
    global existing_messages
    existing_messages.append({"role": "user", "content": prompt})
    truncated_messages = truncate_message_for_token_limit(existing_messages)
    insert_system_message(truncated_messages)
    ...
    llm = AutoModelForCausalLM.from_pretrained(
        "../llama.cpp/models/mistral/ggml-mistral-Q4_K_M.gguf",
        model_type="mistral",
        gpu_layers=50,
        temperature=temperature,
        top_p=0.9,
    ...
    for retry in range(retry_max):
        try:
            if interaction_txt is not None:
                add_to_txt(interaction_txt, ">>> Prompt: \n" + prompt, with_print=False)
            print('START RES')
            res = llm(str(truncated_messages))
            # res = call_res["choices"][0]["message"]["content"]
            existing_messages.append({"role": "assistant", "content": res})
